...
class DemoForm(QMainWindow, form_class):
	def __init__(self):
		...
		self.tableWidget.setHorizontalHeaderLabels(["제품ID", "제품명", "제품가격"])
		
		# 탭키로 네비게이션 금지.
		self.tableWidget.setTabKeyNavigation(False)
		# 엔터키를 클릭하면 다음 컨트롤로 이동하는경우.
	...
	def removeProduct(self):
		...
		con.commit()
	def getProduct(self):
		# 검색 결과를 보여주기전에 기존 컨텐츠를 삭제 (헤더는 제외)
		self.tableWidget.clearContents()
		cur.execute("select * from Products")
		# 행숫자 카운트
		row = 0
		for item in cur:
			int_as_strID = "{:10}".format(item[0]) # 총 10칸의 크기를 가진 문자열로 형변환
